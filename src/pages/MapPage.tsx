  ...
  return (
    <div className="fixed inset-0 pb-16">
      <div className="fixed top-4 left-4 right-4 z-[1000] flex gap-2 flex-wrap">
        <Button onClick={handleLocate} size="sm" className="shadow-lg bg-background">
          <Locate className="w-4 h-4 mr-1" />
          내 위치
        </Button>
        <Button onClick={() => setIsAddingSpot(!isAddingSpot)} size="sm" variant={isAddingSpot ? 'default' : 'outline'} className="shadow-lg bg-background">
          <MapPin className="w-4 h-4 mr-1" />
          {isAddingSpot ? '취소' : '포인트 추가'}
        </Button>
        <Button onClick={() => setShowSpotList(!showSpotList)} size="sm" variant="outline" className="shadow-lg bg-background">
          ...
          <LayersControl position="topright">
            ...
            <LayersControl.Overlay name="해양 정보">
              <WMSTileLayer 
                ...
                params={{ layers: 'KHOA:coastline', format: 'image/png', transparent: true }} 
                attribution="© KHOA" 
              />
